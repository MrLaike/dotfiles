...
drawbar(Monitor *m)
{
    ...
    // fill all bar
	drw_text(drw, 0, 0, m->mw, bh, lrpad / 2, "", 0);
    ...
    char *substr = strtok(bak, "|");
    tw = TEXTW(substr) - lrpad + 2;
    drw_text(drw, x, 0, tw, bh, 0, substr, 0);

    char *substr2 = strtok(NULL, "|");
    tw = TEXTW(substr2) - lrpad + 2;
    drw_text(drw, m->ww - tw, 0, tw, bh, 0, substr2, 0);
    ...
    for (i = 0; i < LENGTH(tags); i++) {
		...
		if (occ & 1 << i)
			drw_rect(drw, x+2, boxs+3, w-3, 1, m == selmon && selmon->sel && selmon->sel->tags & 1 << i, urg & 1 << i);
		x += w;
	}
